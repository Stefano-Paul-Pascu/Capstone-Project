...
public class JwtTools {
	...
	static public UUID extractSubject(String token) {
		String subjectString = Jwts.parserBuilder().setSigningKey(Keys.hmacShaKeyFor(secret.getBytes())).build()
				.parseClaimsJws(token).getBody().getSubject();

		UUID subjectUUID = UUID.fromString(subjectString);
		return subjectUUID;
	}
